...
"""
CLASSIFICATION_DATA = "EXAMPLE.json" ##Sysmon log file with real-data.
MALICIOUS_LOGS ="malicious_logs.json" # The malicious dataset. - Used to measure effectiveness.
BATCH_SIZE = 10000  # Number of logs to process at a time - Used for batch processing in reading large log files.
MODEL_FILE = "Model_File_Generated_During_Training.pkl"  # File to save the trained model
...
Uses the predict method to classify anomalies, and results in an output of anomalies.json
...
def classify_data(model):
    ...
    """
    Classify anomalies based on predict.
    """
    #Call the predict method to classify anomalies.
    classification_df["IsAnomaly"] = model.predict(classification_df[features])
...
#   ################# FILTERING: ###################
    ...
    Not implemented inline but outputted how many anomalies that this would result in filtered.
    """
    ...
    print("Logs that match the filter condition (to be removed):")
    print(filter_df.loc[
        (filter_df["ParentProcessCount"] >= parent_count_threshold) & 
        (filter_df["PairFrequencyRatio"] >= high_ratio_of_lolbas_child)
    ][["ParentProcessCount", "PairFrequencyRatio"]])
